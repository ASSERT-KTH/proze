...
def sanitize_parameter_list(param_list):
  parameters = map(lambda param: "\"" + param + "\"", param_list)
...
def generate_aspects(df, cut):
  ...
  for index, row in df.iterrows():
    if (row['declaringType'] == cut if cut else True):
      with open(template_file_path) as template:
        count += 1
        aspect_string = "\"se.kth.castor.pankti.instrument.plugins.ProzeAspect" + str(count) + "\""
          ...
          for line in template:
            ...
            if ("methodParameterTypes = " in line):
              if not (row['parameters']):
                ...
                param_list = sanitize_parameter_list(row['parameters'])
              line = re.sub(r"=\s{.*},", "= {" + param_list + "},", line)
            if ("timerName = " in line):
              line = re.sub(r"=\s\".+\"\)", "= \"" + row['declaringType'] + "-" + row['name'] + "\")", line)
...
def update_glowroot_plugin_json(aspect_count):
  aspect_path = "se.kth.castor.pankti.instrument.plugins.ProzeAspect"
...
def main(argv):
  try:
    ...
    print("USAGE: python instrument.py </path/to/instrumentation/candidate/list>.json <optional.fqn.of.class.under.test>")
    print(e)
    sys.exit()
